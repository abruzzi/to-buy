...
open class SearchTextField: UITextField {
    ...
    fileprivate func prepareDrawTableResult() {
        guard let frame = self.superview?.convert(self.frame, to: UIApplication.shared.windows.filter {$0.isKeyWindow}.first) else { return }
        if let keyboardFrame = keyboardFrame {
            ...
            if self.center.y + theme.cellHeight > keyWindow!.frame.size.height {
                direction = .up
            } else {
                direction = .down
            }
...
public struct SearchTextFieldTheme {
    ...
    public static func lightTheme() -> SearchTextFieldTheme {
        return SearchTextFieldTheme(cellHeight: 32, bgColor: UIColor (red: 1, green: 1, blue: 1, alpha: 1), borderColor: UIColor (red: 0.9, green: 0.9, blue: 0.9, alpha: 1.0), separatorColor: UIColor.clear, font: UIFont.systemFont(ofSize: 14), fontColor: UIColor.black)
    }
    
    public static func darkTheme() -> SearchTextFieldTheme {
        return SearchTextFieldTheme(cellHeight: 32, bgColor: UIColor (red: 0.8, green: 0.8, blue: 0.8, alpha: 1), borderColor: UIColor (red: 0.7, green: 0.7, blue: 0.7, alpha: 1.0), separatorColor: UIColor.clear, font: UIFont.systemFont(ofSize: 14), fontColor: UIColor.white)
    }
}
